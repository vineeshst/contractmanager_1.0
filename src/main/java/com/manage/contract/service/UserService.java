...
import com.manage.contract.security.AuthoritiesConstants;
import com.manage.contract.security.SecurityUtils;
import com.manage.contract.service.EmailAlreadyUsedException;
import com.manage.contract.service.InvalidPasswordException;
import com.manage.contract.service.UsernameAlreadyUsedException;
import com.manage.contract.service.dto.AdminUserDTO;
import com.manage.contract.service.dto.UserDTO;
...
public class UserService {
    ...
    private final UserSecurityMappingRepository userSecurityMappingRepository;
    ...
    public Mono<User> createUser(AdminUserDTO userDTO) {
        ...
        user.setAuthorisedSignatory(userDTO.isAuthorisedSignatory());
        user.setDepartmentId(userDTO.getDepartmentId());
        user.setSignatoryAgent(userDTO.isSignatoryAgent());
        user.setHierarchyLevel(userDTO.getHierarchyLevel());
        user.setExpiryDate(userDTO.getExpiryDate());
        user.setSupervisorUser(userDTO.getSupervisorUser());
        ...
        return Flux
            ...
            .flatMap(this::saveUser)
            .then(setSecurityGroup(userDTO, user))
            .then(setUserGroup(userDTO, user))
            .then(Mono.just(user))
    ...
    public Mono<User> getUserWithAuthorities() {
        return SecurityUtils
            ...
            .flatMap(
                user -> {
                    if (ObjectId.isValid(user.getId())) return Mono
                        .from(securityGroupRepository.findAllByUserObjectId(user.getId()))
                        .doOnNext(user::setSecurityGroup)
                        .then(Mono.just(user)); else return Mono
                        .from(securityGroupRepository.findAllByUserId(user.getId()))
                        .doOnNext(user::setSecurityGroup)
                        .then(Mono.just(user));
                }
            );
    ...
    private Mono<Void> setUserGroup(AdminUserDTO userDTO, User user) {
        if (userDTO.getUserGroup() != null && userDTO.getUserGroup().getId() != null) return userGroupRepository
            .findById(userDTO.getUserGroup().getId())
            .map(
                userGroup -> {
    ...
    private Mono<Void> setSecurityGroup(AdminUserDTO userDTO, User user) {
        if (userDTO.getSecurityGroup() != null && userDTO.getSecurityGroup().getId() != null) return securityGroupRepository
            .findById(userDTO.getSecurityGroup().getId())
            .map(
                securityGroup -> {
                    securityGroup.getMembers().add(user);
                    return securityGroup;
                }
            )
            .flatMap(securityGroupRepository::save)
            .then();
        return Mono.empty();
    }
}
