...
public class RestApiOAuthTokens {
  ...
  /**
   * TODO
   * Provide an OAuth access token or integration key. If not provided then a new OAuth access token
   * will be requested from AdobeSign server based on credentials provided in the OAuthCredentials.json file. You can also provide a refresh
   * token in OAUTH_REFRESH_TOKEN variable below to refresh the OAuth access token.
  */
  private final static String OAUTH_ACCESS_TOKEN = "";

  /**
   * TODO
   * Provide refresh token here. This will be used to refresh the OAuth access token.
  ...
  public static String getOauthAccessToken(String requestJSONFile) throws Exception {
    ...
    if (!OAUTH_REFRESH_TOKEN.isEmpty())
      return getOauthAccessTokenFromRefreshToken(requestJSONFile);
...
//            "&grant_type=authorization_code";
    ...
    String requestBody = "code=" + "CBNCKBAAHBCAABAA0w3rTQnOIGIMRWSz80luIdy0CQXwZ0vl" +
            "&client_id="+ "CBJCHBCAABAA-_ws1WRb5PBCxJ5XaJPq990ef3g1S4nB" +
            "&client_secret="+ "fkOoovWOr2jnOsF5Hq2Y7tFNDrnCvLqC" +
    ...
    return BEARER + (String) responseJSON.get("access_token");

  }

  private static String getOauthAccessTokenFromRefreshToken(String requestJSONFile) throws Exception{
